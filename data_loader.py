...
def load_dataset(name, expand_features=True):
	...
	if name == "nci1":
		dataset = TUDataset(root="datasets/TUDataset/", name="NCI1", use_node_attr=True)
	elif name == "proteins":
		dataset = TUDataset(root="datasets/TUDataset/", name="PROTEINS", use_node_attr=True)
	elif name == "dd":
		dataset = TUDataset(root="datasets/TUDataset/", name="DD", use_node_attr=True)
	elif name == "enzymes":
		dataset = TUDataset(root="datasets/TUDataset/", name="ENZYMES", use_node_attr=True)
	elif name == "mutag":
		dataset = TUDataset(root="datasets/TUDataset/", name="MUTAG", use_node_attr=True)
	elif name == "collab":
		dataset = TUDataset(root="datasets/TUDataset/", name="COLLAB", use_node_attr=True)
		...
		dataset = TUDataset(root="datasets/TUDataset/", name="REDDIT-MULTI-5K", use_node_attr=True)
...
def build_loader(args, dataset, subset):
	shuffle = (subset != "test")
	loader = DataLoader(dataset_and_aug(dataset, subset, args.augment_list),
...
class dataset_and_aug(Dataset):
	...
	def __init__(self, dataset, subset, augment_list):
		super(dataset_and_aug, self).__init__()
	...
	def get_positive_sample(self, old_graph):
		...
		for function in self.augment_functions:
			new_graph = function.views_fn(new_graph)
		return new_graph
